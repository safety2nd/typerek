...
Supabase exposes no arbitrary-SQL RPC, so do not try to run
`prompts/upcoming-fixtures.sql` over the wire. Instead run a single `Bash`
command that sources `.env.local` and hits the PostgREST read endpoint on the
`fixtures` table:
...
Polish-language queries usually return richer results. If `WebSearch` returns
nothing usable, fall back to `WebFetch` on known URLs (Wikipedia PL,
Transfermarkt PL, the official club site).
